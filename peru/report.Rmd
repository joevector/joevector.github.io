...
datafdir  <- "/home/alulab/Escritorio/bmfazio/"
...
Because `bmi` is the dependent variable, we must exclude any cases without the measurement but we also need to check that doing so may inadvertently bias our data.

```{r}
#Take a look at proportion of missing data 
dhs %T>%
  with(table(year,isNA=is.na(bmi)) %>% prop.table(margin=1) %>% '*'(100) %>% round(2) %>% print) %T>%
  with(table(ethnic_cat,isNA=is.na(bmi)) %>% prop.table(margin=1) %>% '*'(100) %>% round(2) %>% print) %>%
  with(table(residence_cat,isNA=is.na(bmi)) %>% prop.table(margin=1) %>% '*'(100) %>% round(2) %>% print)
```
...
Finally, we remove variables that are no longer necessary.

```{r}
dhs[, c("cluster","hhnumber","hhline","qc_pregnant","qc_visitor","res_ur","res_child","res_prev","ethnic_mom","ethnic_dad","ethnic_self") := NULL]
```
